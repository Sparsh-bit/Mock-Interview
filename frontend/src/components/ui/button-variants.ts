...
/**
 * Button styling, split out of the Button component — components/ui/button-variants.ts
 *
 * WHY IT LIVES IN ITS OWN FILE. `buttonVariants` is a pure `cva()` call — no state, no
 * effects, no browser API. But it used to be exported from `button.tsx`, which carries
 * `'use client'` because the Button component itself uses framer-motion, and a `'use client'`
 * module poisons everything it exports: importing this one function into a SERVER component
 * fails the production build with
 *
 *     Attempted to call buttonVariants() from the server but buttonVariants is on the client.
 *
 * That is what happened writing app/not-found.tsx — a static page with no interactivity, which
 * could not use the app's own button styling without becoming a client component and shipping
 * React to render a 404.
 *
 * `tsc` and ESLint both pass on that code. Only `next build` catches it, which is the reason
 * the build step exists in CI alongside the typecheck.
 *
 * `button.tsx` re-exports this so the ten existing call sites are unchanged.
 */
/**
