...
export class ApiError extends Error {
  ...
  /**
   * The application's OWN error code from the envelope, e.g. `CREDITS_EXHAUSTED`.
   *
   ...
   *
   * It exists because the envelope's `code` was being thrown away entirely — the same
   * oversight the long comment in `normalizeError` describes for `details`. Without it a
   * client can only ever see the HTTP status, so two failures that need completely different
   * handling arrive indistinguishable.
   */
